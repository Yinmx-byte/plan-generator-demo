...
export class HubBridge {
	 ...
	 * @param {(trace: {event: string, data: unknown, timestamp: number}) => void} [onTrace]
	 * @returns {Promise<{success: boolean, data: string}>}
	 */
	async executeTask(task, config, onTrace) {
		...
		return new Promise((resolve, reject) => {
			this.#pendingTask = { resolve, reject, onTrace }
	...
	#onConnection(ws) {
		...
		ws.on('message', (/** @type {Buffer} */ rawData) => {
			...
			if (msg.type === 'result') {
				...
				this.#pendingTask?.onTrace?.({
					event: msg.event ?? 'trace',
					data: msg.data,
					timestamp: Date.now(),
				})
			}
